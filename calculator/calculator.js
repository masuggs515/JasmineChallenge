...
function getCurrentUIValues() {
  return {amount: +(loanAmount.value), years: +(loanYears.value), rate: +(loanRate.value)
  }
}
...
function calculateMonthlyPayment(values) {
  return ((((values.rate/100)/12) * values.amount)/(1- Math.pow((1+((values.rate/100)/12)), -Math.floor(values.years * 12)))).toFixed(2);
}
